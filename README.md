...
# @applaud/cache-loader

The `@applaud/cache-loader` allow to Caches the result of following loaders on disk (default) or in the database.

## Getting Started

To begin, you'll need to install `@applaud/cache-loader`:

```console
npm install --save-dev @applaud/cache-loader
```
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.ext$/,
        use: ['@applaud/cache-loader', ...loaders],
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.js$/,
        use: ['@applaud/cache-loader', 'babel-loader'],
        include: path.resolve('src'),
      },
    ],
  },
